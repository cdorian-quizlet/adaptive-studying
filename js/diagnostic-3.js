...
function setSourceBadge(element) {
    ...
    console.log('🔍 DIAGNOSTIC-3 BADGE DEBUG: Setting badge for element, currentQuestionIndex:', currentQuestionIndex);
    console.log('🔍 DIAGNOSTIC-3 BADGE DEBUG: Current question:', testQuestions[currentQuestionIndex]);
    console.log('🔍 DIAGNOSTIC-3 BADGE DEBUG: window.USING_API_CONTENT:', window.USING_API_CONTENT);
    ...
    console.log('🔍 DIAGNOSTIC-3 BADGE DEBUG: Is API question?', isApiQuestion);
    
    if (isApiQuestion) {
        console.log('🔍 DIAGNOSTIC-3 BADGE DEBUG: Adding API badge');
        element.appendChild(createApiBadge());
    } else {
        console.log('🔍 DIAGNOSTIC-3 BADGE DEBUG: Adding STATIC badge');
    ...
    try {
        ...
        if (diagnosticConfigStr) {
            ...
            if (apiQuestions.length > 0) {
                ...
                console.log(`Using ${testQuestions.length} questions from API for diagnostic 3`);
            } else {
                // Fallback to static questions
                const shuffled = diagnosticQuestions.sort(() => 0.5 - Math.random());
                testQuestions = shuffled.slice(0, 10);
                console.log('Using static questions for diagnostic 3');
            }
        } else {
            // No diagnostic config - use static questions
            const shuffled = diagnosticQuestions.sort(() => 0.5 - Math.random());
            testQuestions = shuffled.slice(0, 10);
            console.log('No diagnostic config - using static questions for diagnostic 3');
        }
    } catch (error) {
